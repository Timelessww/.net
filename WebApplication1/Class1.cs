...
namespace 学生宿舍管理系统
{
    public class Class1
    {
        ...
        public int hsgcaozuojilu(string mren, string mdongzuo, string mbiao)  //自定义函数:执行sql语句,如果执行成功,返回值1,否则0
        {
            //myConnection.Open();
            ...
            SqlConnection conn = new SqlConnection();   //定义新数据库连接
            conn.ConnectionString = connstring;  //设置该新连接字符串是connstr,即上面的webconfig里的cn值
            SqlCommand myCommand = new SqlCommand("insert into caozuojilu(mren,mdongzuo,mbiao) values('" + mren + "','" + mdongzuo + "','" + mbiao + "')", conn); //设置新执行命令
